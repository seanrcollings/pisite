...
data_types = [
	...
	DataType('Current Time', "Current local on the Pi", "date +\%I:\%M\%p"), # displays the time in a human understandable format
	DataType('Current Date', 'Current local date on the Pi', "date +\%d/\%m/\%Y"),
	...
	DataType('Temp', 'The internal temperature of the Raspberry Pi', "/opt/vc/bin/vcgencmd measure_temp | grep -o .....C"),
	DataType('Top PID', 'The PID of the process that is using the most CPU time', "ps -Ao pid --sort=-pcpu | head -n 2 | tr -dc '0-9'"),
	DataType('Process Mem', 'The memory usage of the process that is using the most CPU time', "ps -o pmem --sort=-pmem | head -n 2 | tr -dc '0-9'"), # fixed up some of these commands so they run on the pi
]
