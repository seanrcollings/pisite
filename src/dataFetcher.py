...
#   DataType('Disk Usage', 'How much space is currently taken up on the Pi', "df -h --total | grep total | grep '..%'"),
#	DataType('Internal Temperature', 'The internal temperature of the Raspberry Pi', "/opt/vc/bin/vcgencmd measure_temp | grep .....C"),
# 	DataType('Top Process PID', 'The PID of the process that is using the most CPU time', "ps -Ao pid --sort=-pcpu | head -n 2 | grep -Eo '[0-9]{1-4}'"),
# 	DataType('Top Process Memory Usage', 'The memory usage of the process that is using the most CPU time', "ps -Ao pmem --sort=-pcpu | head -n 2 | grep -Eo '[0-9]{1-4}' | head -n 1")
# ]
